...
export default function Services({ whatsappNumber }: ServicesProps) {
  return (
    <section id="services" className="py-32 bg-brand-dark relative">
      <div className="absolute top-0 left-0 w-full h-px bg-gradient-to-r from-transparent via-brand-primary/20 to-transparent" aria-hidden="true" />
      ...
      <div className="container mx-auto px-4 md:px-6 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
          {services.map((service, index) => (
            <motion.a 
              href={getWhatsAppUrl(whatsappNumber, service.waMessage)}
              target="_blank"
              rel="noreferrer"
            ...
            >
                <div className="absolute inset-0 bg-gradient-to-br from-brand-primary/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 rounded-3xl blur-xl" aria-hidden="true" />
              <div className="relative h-full glass-dark p-8 rounded-3xl border border-brand-light/[0.04] hover:border-brand-primary/25 transition-all duration-500 overflow-hidden flex flex-col">
                <div className="absolute -top-24 -right-24 w-48 h-48 bg-brand-primary/10 rounded-full blur-3xl group-hover:bg-brand-primary/20 transition-all duration-700" aria-hidden="true" />
                ...
                <h3 className="text-2xl font-bold font-serif text-white mb-4 group-hover:text-gradient-gold transition-colors duration-300 tracking-tight">{service.title}</h3>
